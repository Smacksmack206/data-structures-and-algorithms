...
const addTwo = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    let addtwoCallback = arr[i] + 2;
    newarr.push(addtwoCallback);
  }
  return newarr;
};
...
const typeNum = (arr) => {
  const arr2 = arr.filter(arr => typeof arr == 'number');
  return arr2;
...
const notInFirstArray = (forbiddenValues, arr) => {
  arr = [1, 2, 3];
  forbiddenValues = [1, 2, 3, 4];
  const notintheFirst = arr.filter( (arr) => !forbiddenValues.includes(arr));
