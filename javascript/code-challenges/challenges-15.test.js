...
const sortBy = (property, arr) => {
  return arr.filter(objects => objects.price > objects.price || objects.name > objects.name);
};
...
const detectTicTacToeWin = (board) => {
  ...
  if (helpCheck(1, 0, 1, 1, 1, 2)) return true; // mid row
  ...
  if (helpCheck(0, 2, 1, 2, 1, 2)) return true; // right col
